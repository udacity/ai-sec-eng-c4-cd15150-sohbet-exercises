...
| Once you suspect a proxy variable, how do you prove it? You run a counterfactual ablation. Take your declined case and replace that suspect ZIP code with the baseline training median. Re-score the model. If the decision abruptly flips from decline to approve, you have a definitive audit finding. You have proven that feature drove the adverse outcome. | Flow chart. Node 1: 'Declined Case (Suspect ZIP)'. Arrow to Node 2: 'Replace ZIP with Training Median'. Arrow to Node 3: 'Re-score Model'. Arrow to Node 4: 'Result: Approve'. A large label over the final arrow reads 'Decision Flip = Proven Proxy'. [CLICK:] Reveal Node 1 and Node 2; [CLICK:] Reveal Node 3; [CLICK:] Reveal Node 4 and Decision Flip label |
...
| So, how does this framework organize threats? It uses TTPs, which stands for Tactics, Techniques, and Procedures. A Tactic is the overarching goal the adversary wants to achieve, like evading the model's safety controls. A Technique is exactly how they achieve it, such as injecting a malicious prompt. Finally, a Procedure is the documented, real-world instance of this happening to a live production system. | A 3-level pyramid diagram. Top level: 'Tactic' with a small label '(Goal: Evade Safety Controls)'. [CLICK:] Middle level: 'Technique' with a small label '(Method: Prompt Injection)'. [CLICK:] Bottom level: 'Procedure' with a small label '(Real-world Incident)'. Click to reveal Technique; click to reveal Procedure. |
...
| So how does this actually work? The Act uses a risk-based framework. It does not regulate specific technologies like transformers; it regulates the use case. Think of it like city zoning. Some land is unrestricted, some requires basic permits, and some is strictly prohibited to build on. Your system gets classified into one of five tiers, and a single system can sit in multiple zones simultaneously depending on its features. | A risk pyramid showing four levels (Unacceptable, High-Risk, Limited, Minimal), with GPAI as a separate parallel band beside it to signal it is its own regime. Next to it, three 'City Zoning' icons: a green house (unrestricted), a yellow office (permits), and a red factory with a slash (prohibited). [CLICK:] An arrow points from a single 'AI System' node to multiple tiers on the pyramid, labeled 'Context-dependent'. |
| Let us look at the top tier: Unacceptable Risk. Article 5 defines AI practices that are flatly prohibited. This includes social scoring that leads to unjustified harm, untargeted facial-image scraping, and deploying emotion recognition in workplaces or schools. There is no compliance checklist for these. The mandate is simply that you must not deploy them. Remember that emotion-recognition feature your product manager wanted? That falls right here. | A large red stop sign icon labeled 'Unacceptable Risk (Article 5)'. Next to it, a 3-item list with 'X' icons: 'Social Scoring', 'Untargeted Facial Scraping', 'Emotion Recognition (Work/School)'. [CLICK:] A text label appears: 'No Checklist. Do Not Deploy.' |
| Next down is High-Risk, where most engineering and governance work lives. This is triggered by use cases listed in Annex III. If your system makes employment decisions, processes biometrics, or manages critical infrastructure, it lands here. For example, if you build an AI applicant screening tool, that employment use case lands it in Annex III High-Risk — and a finding like an eight percent gender bias then becomes a documented data-governance issue you must address. Have you ever had to defend a model's output to someone who did not trust it? That is exactly the kind of scrutiny High-Risk systems face. | An orange warning triangle labeled 'High-Risk (Annex III)'. Three branching nodes: 'Employment Decisions', 'Biometrics', 'Critical Infrastructure'. [CLICK:] An example box appears under Employment: 'Applicant Screening Tool -> Scrutiny for Bias'. |
| If your system is High-Risk, a standard Model Card is not going to cut it anymore. Article 11 requires a full technical-documentation file, drawn up before your system hits the market, with its contents defined by Annex IV. And behind that documentation sit the real obligations, each in its own article: a risk management system under Article 9, data governance under Article 10, human oversight under Article 14, and post-market monitoring under Article 72. This is a heavy engineering lift, and each requirement has a named owner. | A central block labeled 'Article 11 Technical Documentation (contents per Annex IV)'. Four connected obligation blocks: 'Risk Management (Art 9)', 'Data Governance (Art 10)', 'Human Oversight (Art 14)', 'Post-Market Monitoring (Art 72)'. [CLICK:] A user icon appears next to each block labeled 'Named Owner'. |
| Then we have the General-Purpose AI, or GPAI tier. This covers foundation models. Say you integrate a large language model API into a dispatcher console for natural language queries. Even if the logistics query itself is not High-Risk, that GPAI deployment attaches specific upstream obligations. You have to handle transparency and copyright disclosure — and, for the largest foundation models, systemic risk evaluations. | A central node labeled 'Foundation Model (GPAI)'. An arrow points to a 'Dispatcher Console' node. [CLICK:] Three requirement tags attach to the GPAI node: 'Transparency', 'Copyright Disclosure', 'Systemic Risk Evals (largest models)'. |
...
| Now, the familiar incident response lifecycle still applies, but the actions inside each phase are uniquely AI. Think about containment. You are not quarantining a laptop here. You might be pausing automated routing and falling back to manual dispatch. And for eradication, you are not just patching code. You might have to retrain the model with balanced data to remove data poisoning. | Circular flow diagram showing the incident response lifecycle (Identification, Containment, Eradication, Recovery, Lessons Learned). Two nodes pop out with AI specific labels. 'Containment' points to 'Pause routing -> Manual dispatch'. 'Eradication' points to 'Retrain model with balanced data'. [CLICK:] Show standard IR circular flow; [CLICK:] Highlight Containment node with manual fallback label; [CLICK:] Highlight Eradication node with model retrain label. |
...
| Finally, keep the regulatory clock in mind. An S1 incident clock starts ticking the moment of operational awareness, not when someone finally reads their email. And the clocks differ by regime: a safety-critical transit incident can demand notification within hours, while a personal-data breach under GDPR runs on a seventy-two-hour clock — and the EU AI Act sets its own deadlines in days. A tested AI incident playbook is the only reliable mechanism that bridges the gap between a model failing and a strict regulatory deadline expiring. | Horizontal timeline diagram. Start node on the left: 'Operational Awareness (Clock Starts)'. Timeline moves right to nodes marked '~2 Hours (transit safety)' and '72 Hours (GDPR breach)', each with a gavel icon. A sturdy bridge graphic labeled 'Tested AI Playbook' connects the start node to the deadlines. [CLICK:] Show Operational Awareness starting the clock; [CLICK:] Reveal the 2-hour and 72-hour regulatory deadlines; [CLICK:] Show the 'Tested AI Playbook' bridge connecting the timeline. |
...
| To actually assess an AI vendor, you need a specialized seven-part checklist. Sure, you start with standard items like security posture and data handling. But then you must add model transparency, financial stability, SLA quality, exit strategy, and AI model governance. Think of this like buying a car but demanding to see the exact factory where every single part was made. | Seven-part circular diagram around a central 'AI Assessment' hub. Nodes: 'Security', 'Data Handling', 'Transparency', 'Financial Stability', 'SLA Quality', 'Exit Strategy', 'Governance'. [CLICK:] Reveal 'Security' and 'Data Handling'; [CLICK:] Reveal the five AI-specific nodes. |
...
| Now, how do you turn all these questionnaire answers into an actual decision? You build a risk-weighted scoring model. You score vendors from zero to one hundred across your categories, but you weight them based on your industry. An insurance or financial-services firm might weight data handling especially heavily. Then, you convert that weighted average to a simple one-to-five risk scale. | Flow diagram. Left: 'Raw Scores (0-100)' in multiple boxes. Middle: A multiplier icon 'x Industry Weights (e.g., 50%)'. Right: A final gauge labeled 'Risk Score (1-5)'. [CLICK:] Reveal Raw Scores; [CLICK:] Reveal Industry Weights; [CLICK:] Reveal Final Risk Score. |
| Once you have that one-to-five score, you can drop vendors into four clear recommendation tiers. Under one and a half is highly recommended; up to two and a half, recommended; up to three and a half, acceptable with monitoring; and above three and a half, not recommended — you decline or escalate. But here is the catch: the score alone is not enough. They must also meet your baseline SLA compliance to stay in a recommended tier. | Vertical scale with four bands. Green: 'up to 1.5 Highly Recommended'. Light green: 'up to 2.5 Recommended'. Yellow: 'up to 3.5 Acceptable with Monitoring'. Red: 'above 3.5 Not Recommended'. A padlock icon on the side labeled 'SLA Baseline Prerequisite'. [CLICK:] Reveal the scale and tiers; [CLICK:] Reveal 'SLA Baseline Prerequisite' padlock. |
...
| Ultimately, frameworks like the EU AI Act and GDPR all point to the same reality. In most enterprise integrations — especially once you fine-tune a model or put it to a high-risk use — embedding a foundation model makes you the provider of the integrated system. Your vendor's compliance does not substitute for your own. In the AI supply chain, you are the general contractor. You own the building when it falls down, regardless of which subcontractor made the mistake. | Headshot |
...
| Script *(one row per slide)* | Guidance for Complex Visuals |
| :---- | :---- |
| Your loan-approval model returns a demographic-parity ratio of 0.78. Is it launchable? An engineer says yes, it is the best model yet. A lawyer says no; the EEOC's four-fifths rule of thumb puts the line at 0.80. A product manager says it depends on the accuracy cost to fix it. They are all right. But none of them is the AI Risk Officer. Your job is the interpretation layer: translating that 0.78 into a defensible launch decision. | Central node labeled '0.78 Demographic Parity' with three upward branches and one downward branch. Left branch: Gear icon labeled 'Engineer: Yes - Best Model'. Middle branch: Scales of justice icon labeled 'Lawyer: No - EEOC 4/5ths rule ~0.80'. Right branch: Bar chart icon labeled 'PM: Depends - Accuracy cost'. Downward branch: Shield icon labeled 'AI Risk Officer: Interpretation Layer'. [CLICK:] Show central node '0.78'; [CLICK:] Reveal Engineer branch; [CLICK:] Reveal Lawyer branch; [CLICK:] Reveal Product Manager branch; [CLICK:] Reveal AI Risk Officer branch at the bottom. |
...
| All these decisions must be visible. Your governance needs to be legible through structured outputs, like monthly board updates. These artifacts map directly to external standards like the NIST AI Risk Management Framework and ISO 42001. Above it all sits the principle of effective challenge — a standard drawn from banking's SR 11-7 — meaning your validation process is meaningfully independent from development. This is a gold-standard pattern you will see everywhere. | A mapping diagram. Left side: 'Structured Outputs' (document icons labeled Monthly Board Updates). Arrows pointing to Right side: 'External Standards' (badges labeled NIST AI RMF, ISO 42001). An overarching layer labeled 'Effective Challenge'. [CLICK:] Reveal Structured Outputs; [CLICK:] Draw mapping arrows to External Standards; [CLICK:] Reveal the 'Effective Challenge' overarching layer. |
